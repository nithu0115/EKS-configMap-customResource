...
func Handler(ctx context.Context, event cfn.Event) (physicalResourceID string, data map[string]interface{}, err error) {
	...
	// Get Region info from Resource Properties or lambda env
	region := event.ResourceProperties["AWS_REGION"].(string)
	if len(region) < 1 {
		fmt.Printf("Resource Properties does not have AWS_REGION, pick default from Lambda environment")
